...
void sg_bmap_set_data(
  ...
  bmap->area = area;
  bmap->columns
    = sg_calc_word_width(area.width * SG_BITS_PER_PIXEL_VALUE(bmap));
  bmap->data = mem;
...
size_t sg_calc_bmap_size(const sg_bmap_t *bmap, sg_area_t area) {
  ...
  return sg_calc_word_width(area.width * SG_BITS_PER_PIXEL_VALUE(bmap))
         * area.height * SG_BYTES_PER_WORD;
}
