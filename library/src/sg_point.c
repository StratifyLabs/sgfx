...
static int sign_value(int value) { return (value > 0) - (value < 0); }
...
void sg_point_unmap(sg_point_t *d, const sg_vector_map_t *m) {
  ...
  // api::SgfxObject::api()->point_rotate(d, (SG_TRIG_POINTS - m->rotation) %
  // SG_TRIG_POINTS); map to the space
  s32 tmp_x;
  s32 tmp_y;
  // x' = m->region.point.x + ((x + SG_MAX) * m->region.area.width) /
  // (SG_MAX-SG_MIN); y' = m->region.point.y + ((y + SG_MAX) *
  // m->region.area.height) / (SG_MAX-SG_MIN);

  //(x' - m->region.point.x)*(SG_MAX-SG_MIN)/ m->region.area.width - SG_MAX = x
  // printer().message("%d - %d * %d / %d", d->x, m->region.point.x,
  // (SG_MAX-SG_MIN), m->region.area.width);
  tmp_x = ((d->x - m->region.point.x) * (SG_MAX - SG_MIN)
           + m->region.area.width / 2)
          / m->region.area.width;
  tmp_y = ((d->y - m->region.point.y) * (SG_MAX - SG_MIN)
           + m->region.area.height / 2)
          / m->region.area.height;

  d->x = tmp_x - SG_MAX, d->y = tmp_y - SG_MAX;
}

sg_size_t sg_point_map_pixel_size(const sg_vector_map_t *m) {
  sg_size_t p;
  sg_size_t max = m->region.area.width > m->region.area.height
                    ? m->region.area.width
                    : m->region.area.height;
  s32 tmp;
...
void sg_point_add(sg_point_t *d, const sg_point_t *a) { sg_point_shift(d, *a); }
...
void sg_point_shift_x(sg_point_t *d, sg_int_t a) { d->x += a; }

void sg_point_shift_y(sg_point_t *d, sg_int_t a) { d->y += a; }
