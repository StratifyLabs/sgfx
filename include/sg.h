 ...
 * It stores bitmaps using 1, 2, 4, or 8 bits per pixel (bpp). The bitmap can
 * then be mapped to a display using a color palette. This approach allows
 * powerful graphics to run on devices with very little memory and then to be
 * mapped to color displays.
 *
 ...
 *
 *  The native display requires 32KB of video memory in order to perform fast
 * graphics operations. If you are building a multi-color UI than only uses a
 * few colors, you can use the Stratify Graphics library and represent the UI
 * using 4bpp (16 colors). The UI could be represented in 128*128*4/8 = 8KB of
 * RAM leaving plenty of RAM for other parts of the application.
 *
 * Features
 * ---------------------------
 *
...
static inline void sg_bmap_copy(sg_bmap_t *dest, const sg_bmap_t *src) {
  memcpy(dest, src, sizeof(sg_bmap_t));
}

static inline sg_size_t sg_bmap_margin_left(const sg_bmap_t *bmap) {
  return bmap->margin_top_left.width;
}
static inline sg_size_t sg_bmap_margin_right(const sg_bmap_t *bmap) {
  return bmap->margin_bottom_right.width;
}
static inline sg_size_t sg_bmap_margin_top(const sg_bmap_t *bmap) {
  return bmap->margin_top_left.height;
}
static inline sg_size_t sg_bmap_margin_bottom(const sg_bmap_t *bmap) {
  return bmap->margin_bottom_right.height;
}
static inline sg_int_t sg_bmap_x_max(const sg_bmap_t *bmap) {
  return bmap->area.width - 1;
}
static inline sg_int_t sg_bmap_y_max(const sg_bmap_t *bmap) {
  return bmap->area.height - 1;
}
static inline sg_size_t sg_bmap_h(const sg_bmap_t *bmap) {
  return bmap->area.height;
}
static inline sg_size_t sg_bmap_w(const sg_bmap_t *bmap) {
  return bmap->area.width;
}
static inline sg_size_t sg_bmap_cols(const sg_bmap_t *bmap) {
  return bmap->columns;
}
...
static inline sg_point_t sg_point_origin() {
  sg_point_t p;
  p.point = 0;
  return p;
}
 ...
 *
 */
static inline void sg_cursor_copy(sg_cursor_t *dest, const sg_cursor_t *src) {
  memcpy(dest, src, sizeof(sg_cursor_t));
}
...
/*! \details Draws a horizontal line on \a dest_cursor by copying the values of
 * the \a src_cursor and updates the x location of the \a dest_cursor to the
 * pixel past the end of the operation.
 *
...
sg_int_t
sg_cursor_find_positive_edge(sg_cursor_t *cursor, sg_size_t max_distance);
sg_int_t
sg_cursor_find_negative_edge(sg_cursor_t *cursor, sg_size_t max_distance);
sg_int_t sg_cursor_find_edge(
  sg_cursor_t *cursor,
  sg_color_t current_color,
  sg_size_t max_distance);
 ...
 * @param corners If non-zero, corners[0] will be the top left corner and
 * corners[1] will be the bottom right corner enclosing the curve
 *
 * The color and thickness of the curve are determined by
 * the bmap->pen object.
 *
 */
void sg_draw_quadratic_bezier(
  const sg_bmap_t *bmap,
  sg_point_t p1,
  sg_point_t p2,
  sg_point_t p3,
 ...
 *
 * @param bmap A pointer to the bitmap
 * @param region Specifies the region to draw the ellipse in assuming the
 * rotation is zero
 ...
 */
void sg_draw_bitmap(
  const sg_bmap_t *bmap_dest,
  sg_point_t p_dest,
  const sg_bmap_t *bmap_src);
 ...
 * @return Zero on success
 */
void sg_draw_sub_bitmap(
  const sg_bmap_t *bmap_dest,
  sg_point_t p_dest,
  const sg_bmap_t *bmap_src,
  const sg_region_t *region_src);
...
typedef struct MCU_PACK {
  ...
  void (*cursor_draw_cursor)(
    sg_cursor_t *dest_cursor,
    const sg_cursor_t *src_cursor,
    sg_size_t width);
  void (*cursor_draw_pattern)(
    sg_cursor_t *cursor,
    sg_size_t width,
    sg_bmap_data_t pattern);
  ...
  void (*draw_arc)(
    ...
    sg_point_t *corners);
  void (
    *draw_pour)(const sg_bmap_t *bmap, sg_point_t p, const sg_region_t *region);
  void (*draw_pattern)(
    const sg_bmap_t *bmap,
    const sg_region_t *region,
    sg_bmap_data_t odd_pattern,
  ...
  void (*draw_sub_bitmap)(
    ...
    sg_point_t p_dest,
    const sg_bmap_t *bmap_src,
    const sg_region_t *src_region);

  void (*vector_draw_path)(
    sg_bmap_t *bmap,
    sg_vector_path_t *path,
    const sg_vector_map_t *map);
